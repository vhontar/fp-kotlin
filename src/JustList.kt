...
fun <T> JustList<T>.append2(other: JustList<T>) = foldRight(other) { head, tail -> Construct(head, tail) }

fun <T> JustList<T>.append3(other: JustList<T>) = reversed().foldLeft(other) { tail, head -> Construct(head, tail) }
